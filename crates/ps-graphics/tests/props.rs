...
use ps_graphics::{Graphics, Page, dump, fmt_real};
use ps_vm::{Config, GraphicsBackend, Interp, Io, Matrix, Outcome, Point, Rect, SliceSource};
...
impl Call {
    ...
    fn apply(&self, g: &mut Graphics<Pages>) {
        let _ = match *self {
            ...
            Call::Gray(v) => g
                .set_color_space(&ps_vm::SpaceSpec::DeviceGray)
                .and_then(|()| g.set_color(&[v])),
            Call::Rgb(r, gg, b) => g
                .set_color_space(&ps_vm::SpaceSpec::DeviceRGB)
...
proptest! {
    ...
    fn real_syntax_agrees_with_the_pdf_writer(v in any::<f32>()) {
        prop_assert_eq!(fmt_real(v), pdf_out::fmt_real(v));
    }
