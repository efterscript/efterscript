...
use efterscript_fonts::cff::CffProgram;
use efterscript_fonts::truetype::table_directory;
use efterscript_fonts::{FontError, ResidentFace};
...
fn pagella_opentype_interprets_every_glyph_with_the_metric_tables_advances() {
    ...
    let moves = a
        ...
        .ops
        .iter()
        .filter(|op| matches!(op, efterscript_fonts::OutlineOp::MoveTo(..)))
        .count();
...
fn pagella_opentype_subsets_to_two_glyphs_that_round_trip() {
    use efterscript_fonts::cff::write::{subset, subset_names};
    let Some(bytes) = asset() else {
        return;
    };
    let tables = table_directory(&bytes).expect("an sfnt directory");
    ...
    assert_eq!(
        private.number(efterscript_fonts::cff::op::STD_VW),
        full.number(efterscript_fonts::cff::op::STD_VW)
    );
    assert_eq!(again.font_bbox(), cff.font_bbox());
    assert_eq!(
        again.top_string(efterscript_fonts::cff::op::NOTICE),
        cff.top_string(efterscript_fonts::cff::op::NOTICE)
    );
    println!(
