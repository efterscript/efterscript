...
use efterscript_graphics::{FillRule, Graphics, IrOp, Page, SpaceRef, dump};
use efterscript_vm::{Config, Interp, Io, Matrix, Outcome, Point, Seg, SliceSource, SpaceSpec};
...
use efterscript_graphics::{FontIndex, FontSpec};
...
fn text_operation_shape() {
    ...
    assert!(matches!(
        ...
        FontSpec::Resident { base: efterscript_fonts::ResidentFace::Helvetica, encoding }
...
use efterscript_fonts::testing::{corpus_cid_cff, corpus_cmap};
...
fn a_simple_descendant_shows_as_that_font_with_the_cid_as_its_code() {
    ...
    assert!(matches!(
        &page.resources.fonts[0],
        FontSpec::Resident {
            base: efterscript_fonts::ResidentFace::Helvetica,
            ..
        }
    ));
    assert_eq!(
