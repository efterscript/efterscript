...
use efterscript_fonts::{Program, ProgramKind, ResidentFace};
use efterscript_vm::{
...
/// they come from the metrics in `efterscript-fonts` by glyph name; an embedded
...
impl FontSpec {
    ...
    pub fn cid_glyph(&self, cid: u16) -> Option<Rc<efterscript_fonts::Glyph>> {
    ...
    pub fn program_glyph(&self, code: u8) -> Option<Rc<efterscript_fonts::Glyph>> {
...
mod tests {
    ...
    fn shadings_are_interned_by_value() {
        let axial = |extend: bool| ShadingSpec {
            kind: efterscript_vm::ShadingKind::Axial {
                coords: [0.0, 0.0, 10.0, 0.0],
                domain: [0.0, 1.0],
                function: vec![efterscript_vm::FunctionSpec::Exponential {
                    domain: vec![0.0, 1.0],
                    range: Vec::new(),
                    c0: vec![0.0],
                    c1: vec![1.0],
    ...
    fn embedded_fonts_measure_through_their_program_and_compare_by_snapshot() {
        use efterscript_fonts::testing::{TrueTypeFont, Type1Font, rectangle};
