...
use ps_fonts::cidfont::{CidLayout, FdLayout, SubrSource, Type1CidProgram};
use ps_fonts::cmap::{CMap, CMapBuilder, CMapError, CidSystemInfo};
use ps_fonts::{Program, type1};
...
pub(crate) fn resolve(i: &mut Interp, operand: Object, retry: &str) -> Result<Resolved, VmError> {
    ...
    let Some(text) = ps_fonts::cmap::predefined(&name) else {
