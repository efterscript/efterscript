...
//! file closes, since `codec`'s encoders take a complete buffer.
...
impl Encoder {
    ...
    pub(crate) fn finish(&mut self, out: &mut Vec<u8>) {
        match self {
            ...
            Encoder::Flate(buffer) => out.extend(codec::deflate::compress(buffer)),
            Encoder::Lzw {
                buffer,
                early_change,
            } => out.extend(codec::lzw::encode(buffer, *early_change)),
            Encoder::Null => {}
        }
    }
}
