...
use pdf_out::{Document, Error, Filter, PageTree, Ref, write_info};
...
fn nul_in_names_is_rejected() {
    let mut out = Vec::new();
    let mut doc = pdf_out::Document::new(&mut out).expect("header");
    let r = doc.alloc();
    assert!(matches!(
        doc.write_obj(r, |v| v.name_bytes(b"bad\0name")),
        Err(pdf_out::Error::NulInName)
    ));
    let r2 = doc.alloc();
    assert!(matches!(
    ...
    let r3 = doc.alloc();
    assert!(matches!(
        doc.write_stream(r3, pdf_out::Filter::None, b"x", |d| {
