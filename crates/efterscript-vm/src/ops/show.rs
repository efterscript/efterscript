...
use efterscript_fonts::{CMap, Glyph as ProgramGlyph, OutlineOp, Program, ResidentFace};
...
mod tests {
    ...
    fn em_units_follow_the_program_kind() {
        let cff = efterscript_fonts::testing::corpus_cff().program().unwrap();
        assert_eq!(em_units(&cff, 1.0), 1000.0);
        let tt = efterscript_fonts::testing::corpus_truetype()
            .program()
            .unwrap();
