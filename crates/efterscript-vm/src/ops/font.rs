...
use efterscript_fonts::{Encoding, ResidentFace};
...
pub(crate) fn find(i: &mut Interp, key: Object) -> Result<Object, VmError> {
    ...
    let face = match ResidentFace::from_postscript_name(&text) {
        ...
        None if i.fonts_config.substitute => {
            let face = efterscript_fonts::substitute(&text);
...
mod tests {
    ...
    fn encoding_arrays_fill_gaps_with_notdef() {
        ...
        let array = encoding_array(&mut mem, &efterscript_fonts::STANDARD_ENCODING).unwrap();
