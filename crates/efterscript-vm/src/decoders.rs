...
use efterscript_codec::inflate::Inflater;
use efterscript_codec::predictor::Predictor;
use efterscript_fonts::type1::{Decryptor, EEXEC_KEY, hex_value};
...
impl Decoder {
    ...
    pub(crate) fn lzw(early_change: bool, predictor: Option<Predictor>) -> Self {
        Decoder::Lzw(Box::new(Lzw {
            decoder: efterscript_codec::lzw::Decoder::new(early_change),
            predictor,
            scratch: Vec::new(),
        }))
    }
...
impl Flate {
    fn push(&mut self, byte: u8, out: &mut Vec<u8>) -> Result<Fed, VmError> {
        ...
        Ok(match status {
            efterscript_codec::inflate::Status::More => Fed::More,
            efterscript_codec::inflate::Status::Done => {
                self.finish(out);
                Fed::End
            }
        })
...
pub(crate) struct Lzw {
    decoder: efterscript_codec::lzw::Decoder,
    predictor: Option<Predictor>,
    scratch: Vec<u8>,
}
...
impl Lzw {
    fn push(&mut self, byte: u8, out: &mut Vec<u8>) -> Result<Fed, VmError> {
        ...
        predicted(&mut self.predictor, &mut self.scratch, out);
        Ok(match status {
            efterscript_codec::lzw::Status::More => Fed::More,
            efterscript_codec::lzw::Status::Done => {
                self.finish(out);
                Fed::End
            }
        })
...
mod tests {
    ...
    fn flate_and_lzw_end_with_their_streams() {
        ...
        let z = efterscript_codec::deflate::compress(&rows);
    ...
    fn eexec_hex_ends_before_a_foreign_byte() {
        let cipher = efterscript_fonts::type1::encrypt(EEXEC_KEY, b"ab", 4);
