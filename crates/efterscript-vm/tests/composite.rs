...
use efterscript_fonts::testing::{CidType1Font, corpus_cid_cff, corpus_cmap, corpus_truetype};
use efterscript_vm::{Config, FontSource, Glyph, Interp, Io, Outcome, Point, SliceSource};
...
fn cmap_of(interp: &mut Interp, dict: efterscript_vm::Object) -> Rc<efterscript_fonts::CMap> {
...
fn hexadecimal_glyph_data_is_read_too() {
    ...
    program.push_str(&efterscript_fonts::testing::hex_lines(&data));
    program.push_str(
        "/T /Identity-H [ /SynCIDT1 /CIDFont findresource ] composefont 10 scalefont setfont \
         <0002> stringwidth",
    );
...
fn a_simple_descendant_takes_the_cid_as_its_code() {
    ...
    assert!(matches!(
        &run.sources()[0],
        FontSource::Composite { descendant, .. }
            if matches!(**descendant, FontSource::Resident(efterscript_fonts::ResidentFace::Helvetica))
    ));
    let run = exec(
