...
use efterscript_graphics::{DocMark, IrOp, Page, PageSink};
use efterscript_remelt::{Options, PdfSink, Report};
use efterscript_vm::{Bounds, Config, Io, MarkValue, Outcome, Point, Seg};
use support::{Value, check, decoded, font, page, resources, xobject};

fn config() -> (Config, efterscript_vm::Capture) {
...
fn run(program: &str, options: &Options) -> (Report, Vec<u8>, String) {
    ...
    let (report, pdf) =
        efterscript_remelt::distill(program.as_bytes(), config, options, Vec::new()).unwrap();
    (report, pdf, out.text())
}

fn run_seekable(program: &str, options: &Options) -> (Report, Vec<u8>) {
    let (config, _) = config();
    let (report, cursor) = efterscript_remelt::distill_seekable(
        program.as_bytes(),
        config,
        options,
        Cursor::new(Vec::new()),
...
fn a_params_mark_replayed_into_the_sink_applies_to_later_pages() {
    ...
    page.ops.push(
        IrOp::Fill {
            ...
            rule: efterscript_graphics::FillRule::NonZero,
        }
        .into(),
    );
