...
use efterscript_remelt::{Options, Params, Report};
use efterscript_vm::{Config, Io, Outcome};
use support::{
...
fn distil_with(program: impl AsRef<[u8]>, options: &Options) -> Run {
    ...
    let (report, pdf) =
        efterscript_remelt::distill(program.as_ref(), config, options, Vec::new()).unwrap();
    Run {
        report,
        pdf,
        output: out.text(),
...
fn the_report_carries_the_identity_and_a_prelude_failure_is_an_error() {
    ...
    let config = Config {
        ...
        identity: vec![(
            "product".to_string(),
            efterscript_vm::MarkValue::String(b"Fictional Press".to_vec()),
        )],
        prelude: Some(b"statusdict /waittimeout 300 put".to_vec()),
        ..Default::default()
    };
    let (report, _) = efterscript_remelt::distill(
    ...
    let error = efterscript_remelt::distill(b"1", config, &Options::compress(false), Vec::new())
        .expect_err("fails");
    assert!(matches!(&error, efterscript_remelt::Error::Prelude(e) if e.name == "undefinedresult"));
...
use efterscript_vm::FontSubstitution;
...
use efterscript_fonts::TrueTypeProgram;
use efterscript_fonts::testing::{corpus_truetype, corpus_type1};
use efterscript_fonts::type1::decrypt_section;
use efterscript_graphics::{Graphics, IrOp, Page};
use efterscript_vm::{Interp, SliceSource};
...
fn a_type1c_subset_round_trips_through_the_engine() {
    use efterscript_fonts::CffProgram;
    use efterscript_fonts::testing::corpus_cff;
...
use efterscript_fonts::testing::{CidType1Font, corpus_cid_cff, corpus_cmap};
...
fn a_cidfonttype2_descendant_carries_a_cid_to_gid_map() {
    use efterscript_fonts::testing::corpus_truetype;
    let run = distil(&format!(
