...
use efterscript_vm::{Bounds, Matrix, ShadingSpec, SpaceSpec};
...
) -> Result<(Vec<Ref>, Recode), efterscript_pdf::Error> {
...
fn to_unicode<'a>(
    ...
    let mapped: Vec<(u32, Vec<char>)> = entries
        .filter_map(|(code, name)| {
            efterscript_fonts::unicode(name)
...
pub(crate) fn write_to_unicode_codes<W: Write>(
    ...
    len: u8,
    mapped: &[(u32, Vec<char>)],
) -> Result<Option<Ref>, efterscript_pdf::Error> {
    if mapped.is_empty() {
        return Ok(None);
    }
    let r = doc.alloc();
...
fn write_type3<W: Write>(
    ...
    recode: &Recode,
    filter: Filter,
) -> Result<Vec<String>, efterscript_pdf::Error> {
...
mod tests {
    ...
    use efterscript_graphics::FillRule;
    use efterscript_vm::Seg;
