...
use efterscript_graphics::{Image, IrOp, Op, Page, Resources};
use efterscript_vm::{Encoded, ImageSpec, Matrix, SpaceSpec};
...
mod tests {
    ...
    fn painted_matrices_are_gathered_by_resource_index() {
        let mut page = Page::new(efterscript_vm::Bounds::new(0.0, 0.0, 100.0, 100.0));
        let first = page
            .resources
        ...
        .into_iter()
        .map(efterscript_graphics::Op::from)
        .collect();
    ...
    fn images_inside_forms_and_patterns_are_measured_where_they_land() {
        use efterscript_graphics::{FormSpec, IrOp, PatternSpec};
        let mut page = Page::new(efterscript_vm::Bounds::new(0.0, 0.0, 100.0, 100.0));
        let in_form = page
            .resources
            .add_image(&spec(Some(SpaceSpec::DeviceGray), 8, 1, 1), &[0]);
        ...
        let cell = page.resources.add_pattern(PatternSpec::Tiling {
            ...
            bbox: efterscript_vm::Bounds::new(0.0, 0.0, 1.0, 1.0),
            xstep: 1.0,
            ystep: 1.0,
            paint_type: 1,
            tiling_type: 1,
        ...
        let inner = page.resources.add_form(FormSpec {
            bbox: efterscript_vm::Bounds::new(0.0, 0.0, 10.0, 10.0),
            ops: vec![
                IrOp::Image {
                    image: in_form,
