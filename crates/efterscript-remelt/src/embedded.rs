...
use efterscript_fonts::type1::write::{Header, subset_names};
use efterscript_fonts::{
    Program, ProgramKind, STANDARD_ENCODING, TrueTypeProgram, Type1Program, cff, type1,
};
use efterscript_graphics::{FontSpec, GlyphNames, IrOp, Op, Page};
use efterscript_pdf::{Document, Filter, Ref};
use efterscript_vm::Matrix;
...
pub(crate) fn pdf_bbox(bbox: [f32; 4], matrix: Matrix) -> [f32; 4] {
    ...
    for (x, y) in corners {
        let p = matrix.apply_delta(efterscript_vm::Point::new(x, y));
...
pub(crate) fn write_font<W: Write>(
    ...
    let (base_font, descriptor) = match &*program.0 {
        ...
        Program::TrueType(truetype) => {
            ...
            let name = embedded_name(subset, font_name, &keep);
            let bytes = efterscript_fonts::truetype::write::subset(truetype, &gids, &cmap)
