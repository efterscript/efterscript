...
- [Viewers differ on `CIDToGIDMap` and `W` edge cases] → the external checker's font listing, text extraction, and rasterising run on every composite
...
- **Type 0 output (D7) as built.** `remelt::composite`, sharing
  ...
  code map (an empty `(3,0)` subtable — the external checker reads the font without
...
- **Corpus.** `corpus/unit/fonts/`: `composite-two-byte-width`
  ...
  with `.ir` and `.pdf` goldens. The external PDF checker on every composite golden:
  the external checker's font listing lists `CID Type 0C … Identity-H emb yes sub yes` (`uni yes`
  for the UCS2 file, `Identity-V` for the vertical one), `CID TrueType
  … Identity-H emb yes sub yes` for the CIDFontType2, and `Type 3
  Custom` for the fallback; the external checker accepts all 138 corpus documents
  as `EFTERSCRIPT_PDF_CHECK`; its text extraction extracts `AB` from the UCS2
  golden (and the CIDs as characters where no ToUnicode exists);
  its rasteriser renders every file, the vertical glyph at (−2..2,
...
- **Verification.** `cargo test --workspace`: 645 passed, 2 ignored
  ...
  `EFTERSCRIPT_PDF_CHECK` set to the external checker; `parse-survival` 138 files, no
