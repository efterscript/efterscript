...
## Purpose
Defines how EfterScript turns bytes into PostScript objects: the token
classes and their attributes, the exact stream position left after each
token (so operators can read raw data from the same source), resumable
scanning of chunked input, error reporting with spans, allocation through
the VM's current mode, and the DSC comment observer. Token syntax is the
PostScript Language Reference's; this spec records the behaviours the
