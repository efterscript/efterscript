...
Verdicts SHALL be `pass`, `fail`, and `expected-divergence`. A corpus
file carrying `% divergence: <slug>` SHALL be reported as
...
run when a slug has no such requirement. The summary SHALL count each
