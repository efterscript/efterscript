...
//! SHA-256 (FIPS 180-4), written out so the fetch tool needs no
//! dependency; the same digest the provenance test computes.
...
/// The digest of `data` as lower-case hexadecimal.
...
mod tests {
    ...
    fn known_vectors() {
        ...
        // Two blocks, with the length landing in the second.
        assert_eq!(
            hex_digest(&[b'a'; 64]),
